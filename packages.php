  ...
  if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (isset($_POST["load_add_section_modal"])) {
      ...
      "sofa"=>"Sofas","tv"=>"TVs","light"=>"Lighting",
        ...
        "ac"        => [
          ...
          "air_curtain"=>"Air Curtains","ceiling_fan"=>" Fans"
        ],
      ];
    ...
    if (isset($_POST["add_section_item"])) {
      ...
      if ($res && ($row = pg_fetch_assoc($res))) {
        if (isset($_SESSION["wizard"][$cartKey]["items"][$type])) {
          // Section already has a recommended product — add as extra instead
...
if (($_SESSION["wizard"]["furniture_cart_tier"] ?? null) !== $furnitureTier
  ...
  if ($posCap > 0 && !isset($_SESSION["wizard"]["pos_preference"])) {
    if (in_array($restaurantType, ["cloud_kitchen", "fast_food"], true)) {
            ...
            <div class="sf-module-shell">
              ...
              <?php else: ?>
                ...
                foreach ($missingPosTypes as $mt) {
                  ...
                  $userShownPos = $_SESSION["wizard"]["shown_sections"]["pos"] ?? [];
                  if (!in_array($mt, $_SESSION["wizard"]["hidden_sections"]["pos"], true)
                      && !in_array($mt, $userShownPos, true)) {
                ...
                <?php foreach($missingPosTypes as $type): ?>
  <?php if(in_array($type, $_SESSION["wizard"]["shown_sections"]["pos"] ?? [])): ?>
          ...
          <?php elseif ($activeModule === "kitchen"): ?>
            <div class="sf-module-shell">
              ...
              <?php else: ?>
                ...
                <?php foreach($missingKitchenTypes as $type): ?>
                <?php if(in_array($type, $_SESSION["wizard"]["shown_sections"]["kitchen"] ?? [])): ?>
                <div class="sf-pkg-sections" style="margin-top:0;">
                <div class="sf-pkg-section" data-module="kitchen" data-type="<?= htmlspecialchars($type) ?>">
                  <div class="sf-pkg-section-head">
                    <h4 class="sf-pkg-section-title">
    ...
    <div class="sf-module-shell">
      ...
      <?php else: ?>
        ...
        foreach ($missingFurnitureTypes as $mt) {
          ...
          $userShownFurniture = $_SESSION["wizard"]["shown_sections"]["furniture"] ?? [];
          if (!in_array($mt, $_SESSION["wizard"]["hidden_sections"]["furniture"], true)
              && !in_array($mt, $userShownFurniture, true)) {
        ...
        <?php foreach($missingFurnitureTypes as $type): ?>
        <?php if(in_array($type, $_SESSION["wizard"]["shown_sections"]["furniture"] ?? [])): ?>
        <div class="sf-pkg-sections" style="margin-top:0;">
        <div class="sf-pkg-section" data-module="furniture" data-type="<?= htmlspecialchars($type) ?>">
          <div class="sf-pkg-section-head">
            <h4 class="sf-pkg-section-title">
      ...
      .then(data => {
        ...
        tiers.forEach(function(tier) {
          ...
          tierProducts.forEach(function(p) {
            ...
            html += '<input type="hidden" name="product_type" value="' + (filterType || p.product_type) + '">';
  ...
  <div id="sf-pos-popup" style="position:fixed;inset:0;z-index:9000;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,.55);padding:16px;">
    <div style="background:#fff;width:100%;max-width:480px;padding:32px 28px 28px;position:relative;">
      <h2 style="font-size:17px;font-weight:900;color:#111827;margin:0 0 6px;">POS Setup</h2>
      <p style="font-size:13px;color:#6b7280;margin:0 0 24px;">Do you want ordering tablets for your waitstaff?</p>

      <form method="post" id="sf-pos-pref-form">
        <input type="hidden" name="save_pos_preference" value="1">
        <input type="hidden" name="pos_device" value="terminal">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:28px;">
          <label class="sf-pos-opt" for="svc_tab_no" style="cursor:pointer;">
            ...
            <div class="sf-pos-opt-card" data-for="svc_tab_no" style="border:2px solid #004cac;padding:20px 12px;text-align:center;transition:all .15s;">
              <i class="bi bi-display" style="font-size:28px;color:#004cac;display:block;margin-bottom:8px;"></i>
              <div style="font-size:13.5px;font-weight:800;color:#111827;">No Tablets</div>
              <div style="font-size:11.5px;color:#6b7280;margin-top:4px;">Waitstaff takes orders at the counter or manually</div>
            </div>
          </label>
          <label class="sf-pos-opt" for="svc_tab_yes" style="cursor:pointer;">
            ...
            <div class="sf-pos-opt-card" data-for="svc_tab_yes" style="border:2px solid #e5e7eb;padding:20px 12px;text-align:center;transition:all .15s;">
              <?php if ($popupTablet && !empty($popupTablet['image_url'])): ?>
                <img src="<?= htmlspecialchars($popupTablet['image_url']) ?>" alt="" style="width:100%;height:80px;object-fit:contain;margin-bottom:6px;">
              <?php else: ?>
                <i class="bi bi-tablet" style="font-size:28px;color:#6b7280;display:block;margin-bottom:8px;"></i>
              <?php endif; ?>
              <div style="font-size:13.5px;font-weight:800;color:#111827;">Add Tablets</div>
              <?php if ($popupTablet): ?>
              ...
              <?php else: ?>
                <div style="font-size:11.5px;color:#6b7280;margin-top:4px;">Portable tablets for table-side ordering</div>
              <?php endif; ?>
            </div>
          </label>
        </div>

        <button type="submit" style="width:100%;padding:14px;background:#004cac;color:#fff;font-weight:800;font-size:15px;border:none;cursor:pointer;">
          Build My POS Setup
        </button>
      </form>
    </div>
